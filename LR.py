...
def scoring_function_factory(Nconf,Nw,Nv,N_res):
    def eval(chromosome):
        v = chromosome[0]
        w = chromosome[1]*10
        Z = make_partition(N_res,v=v,w=w)
        
        lnL = Nw * numpy.log(w) + Nv * numpy.log(v) - Nconf * numpy.log(Z)
...
def main(f_phi,f_psi):
    ...
    eval_func = scoring_function_factory(number_conf,number_helical_w,number_helical_v,n_res)
